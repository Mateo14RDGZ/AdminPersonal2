    ...
    <div ref={stageRef} className={`assistant-mascot-stage ${fullScreen ? "assistant-mascot-stage-full" : ""} ${className}`} data-behavior={brain.behavior} data-emotion={brain.emotion} data-mood={brain.mood} data-attention={brain.attention}>
      ...
      <motion.div className="assistant-mascot-motion" style={{ x: brain.x, y: brain.y, rotate: brain.rotation }}>
        <motion.div className="assistant-mascot-flight-shape" style={{ scaleX: brain.stretchX, scaleY: brain.stretchY, skewX: brain.flightSkew }}>
          ...
          <motion.div
            animate={reducedMotion ? { opacity: 1 } : brain.behavior === "celebrating" ? { y: [0, -14, 0, -7, 0], rotate: [0, -6, 6, 0], scale: [1, 1.09, .99, 1.04, 1] } : brain.behavior === "recovering" ? { x: [0, -5, 6, -2, 0], scale: [1, .97, 1.01, 1] } : brain.behavior === "thinking" ? { y: [0, -3, -7, -3, 0], rotate: [0, -2.4, 1.6, 0], scale: [1, .995, 1.012, 1] } : brain.behavior === "speaking" ? { y: [0, -3.5, 0, -1.5, 0], rotate: [0, 1.4, -1, 0], scale: [1, 1.018, .997, 1] } : brain.behavior === "sleeping" ? { y: [0, -2, 0], rotate: [0, .4, 0], scale: [1, .995, 1] } : { y: [0, -4, 0, -2, 0], rotate: [0, .8, -.65, 0], scale: [1, 1.015, .997, 1] }}
            transition={{ duration: brain.behavior === "speaking" ? .76 : brain.behavior === "thinking" ? 1.36 : brain.behavior === "sleeping" ? 3.2 : 2.2, repeat: ["celebrating", "recovering"].includes(brain.behavior) || reducedMotion ? 0 : Infinity, ease: "easeInOut" }}
          >
